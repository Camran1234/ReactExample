...
import logo from '../assets/image/logoEmpresa.png';
...
const LoginEmpresa = (props) => {
    ...
    const handleSubmit = (event) => {
        ...
        fetch('http://localhost:4000/api/loginEmpresa', {
          method: 'POST',
          headers: {
        ...
        .then(response => {
            if (response.status === 401) {
                ...
                response.json().then(data => {
                    localStorage.setItem("idEmpresa",data.message);
                });
            }
        })
